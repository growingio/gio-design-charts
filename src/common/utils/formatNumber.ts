const formatNumber = (value: any, decimalCount: number = 2, intSuffixZeroFill: boolean = false) => {
    if (!isFinite(value)) {
      return value + '';
    }
  ...
  export const parseFormatedNumber = (text: string) => parseFloat(text.split('').filter((c) => c !== ',').join(''));
  export const formatPercent = (value: string | number, decimalCount?: number, intSuffixZeroFill: boolean = true) => typeof value === 'number' ? formatNumber((value || 0) * 100, decimalCount, intSuffixZeroFill) + '%' : value
